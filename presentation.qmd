...
- then we used the `s()` slector to get an elemtent
- next we took that cookie element `cookiebanner` 
- checked wether it is there, and then clicked it with `elem_click`
:::
:::
:::
...
Great! Now that we handled that we can move to the fun part. 
